...
void addEmployee(tree_node_t* root, tree_node_t* employeeNode){
    if(root == NULL){
        return createEmployeeNode(root, employeeNode);
    }
    if(employeeNode->data.number < root->data.number){
        root->left = addEmployee(root->left, employeeNode);
    }
    else{
        root->right = addEmployee(root->right, employeeNode);        
    }
...
void updateEmployeeName(tree_node_t* root, int number, char newName[MAX_LEN]){
    node_data_t tempEmp = findEmployeeByNumber(root, number);
    strcpy(tempEmp->name, newName);
...
void removeEmployee(tree_node_t* root, int number){
    ...
    if(number < root->data->number){
        root->left = removeEmployee(root->left, number);
    }
    else if(number > root->data->number){
        root->right = removeEmployee(root->right, number);
    }
...
int main() {
    ...
    /*
        for (int i = 0; i < 5; i++) {
            printf("%d %s\n", employeesData[i].number, employeesData[i].name);
        }
     */
